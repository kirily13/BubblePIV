...
%Section 5.5: Raw Calculations via Bubble Tracking
    if detectedFrames(bfi) == true
        ...
        [yPix, xPix] = find(bubbleMask(:,:,bfi));
        xCentroid(bfi) = mean(xPix);
        yCentroid(bfi) = mean(yPix);
        bubbleVolumeFrame = 0;
        yPixLoop = unique(yPix)';
        %diameter calculation via disk method
        for bfiDiameter = yPixLoop
            yDiaIndex = find(yPix == bfiDiameter);
            xPixTemp = xPix(yDiaIndex);
            radius = (max(xPixTemp) - min(xPixTemp) + 1) / 2;
            bubbleVolumeFrame = bubbleVolumeFrame + (pi * radius^2);
        end
        bubbleVolumePixels(bfi) = bubbleVolumeFrame;
        bubbleDiameterPixels(bfi) = ((6*bubbleVolumePixels(bfi)) / pi)^(1/3);
    end
end
...
title('Centroid Trajectories of Bubbles');
grid on;
...
xlabel('V_y Centroid (m/s)');
ylabel('Y Centroid (mm)');
title('Y Centroid Versus V_y Centroid');
grid on;

%Accel X versus Y Centroid
figure;
plot(xAccelerationCentroid, yCentroidMM, 'r-');
xlabel('A_x Centroid (m/s^2)');
ylabel('Y Centroid (mm)');
title('Y Centroid Versus A_x Centroid');
grid on;

%Accel Y versus Y Centroid
figure;
plot(yAccelerationCentroid, yCentroidMM, 'r-');
xlabel('A_y Centroid (m/s^2)');
ylabel('Y Centroid (mm)');
title('Y Centroid Versus A_y Centroid');
grid on;
...
Reynolds_number = (liquidDensity * mean(VelocityTotal,'omitnan') * mean(bubbleDiameterMM/1000,'omitnan')) / dynamicViscosity;
Etovos_number = (((liquidDensity-gasDensity)) * gravity * (mean(bubbleDiameterMM/1000,'omitnan'))^2) / surfaceTension;
Weber_number = (liquidDensity * (mean(VelocityTotal,'omitnan'))^2 * mean(bubbleDiameterMM/1000,'omitnan')) / surfaceTension;

resultsTable = table(Reynolds_number, Etovos_number, Weber_number, xVelocityAvg, yVelocityAvg, xAccelerationAvg, yAccelerationAvg,...
    'VariableNames', {'Reynolds Number', 'Etovos Number','Weber Number', 'Avg X Velocity','Avg Y Velocity','Avg X Acceleration','Avg Y Acceleration'});
disp(resultsTable)